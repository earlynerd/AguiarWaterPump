...
WaterPump pump(pump_TachPin, pump_PwmPin, pump_PowerPin, 1);
...
void setup()
{
  ...
  pump.begin();
  attachInterrupt(digitalPinToInterrupt(pump_TachPin), pumpInterruptWrapper, CHANGE);
...
void loop()
{
  // countPulseRate();
  // pump.update();
  if(!pump.isEnabled()) ledControl(false);
  ArduinoOTA.handle();
}
...
void blynkUpdateChart(void *pvParams)
{
  for (;;)
  {
    Blynk.virtualWrite(V4, pump.getSpeed(300000UL));
    vTaskDelay(100);
  }
}
